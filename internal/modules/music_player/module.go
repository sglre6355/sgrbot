...
func (m *MusicPlayerModule) initWithLavalink(deps bot.ModuleDependencies) error {
	...
	// Domain services
	autoPlayService := core.NewAutoPlayService(core.UserID(session.State.User.ID), trackRecommender)
	playerService := core.NewPlayerService(autoPlayService)
	...
	findPlayerState := usecases.NewFindPlayerStateUsecase(
		voiceConnectionGateway,
	)
	...
	setNotificationChannel := usecases.NewSetNotificationChannelUsecase(
		nowPlayingGateway,
	)
	...
	if err := errors.Join(
		m.eventBus.Subscribe(
			reflect.TypeFor[core.TrackStartedEvent](),
			domainEventHandlers.HandleTrackStarted,
		),
		m.eventBus.Subscribe(
			reflect.TypeFor[core.TrackEndedEvent](),
			domainEventHandlers.HandleTrackEnded,
		),
		m.eventBus.Subscribe(
			reflect.TypeFor[core.QueueExhaustedEvent](),
			domainEventHandlers.HandleQueueExhausted,
		),
		m.eventBus.Subscribe(
			reflect.TypeFor[core.PlaybackStoppedEvent](),
