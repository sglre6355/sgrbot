...
var _ core.TrackRecommender = (*YouTubeTrackRecommender)(nil)
...
func (a *YouTubeTrackRecommender) AcceptsSeed(seed core.QueueEntry) bool {
	return seed.Track().Source() == core.TrackSourceYouTube
}
...
func (a *YouTubeTrackRecommender) GetRecommendation(
	ctx context.Context,
	seeds []core.QueueEntry,
	exclusions []core.QueueEntry,
	limit int,
) ([]core.Track, error) {
	...
	excludeSet := make(map[core.TrackID]struct{}, len(seeds)+len(exclusions))
	...
	trackScores := make(map[core.TrackID]*scoredTrack)
	...
	result := make([]core.Track, 0, limit)
	for i := range ranked {
		if len(result) >= limit {
			break
		}
