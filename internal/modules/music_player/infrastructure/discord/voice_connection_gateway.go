...
const voiceConnectionTimeout = 2 * time.Second
...
var (
	...
	_ ports.PlayerStateLocator[discord.PartialVoiceConnectionInfo] = (*DiscordVoiceConnectionGateway)(
		nil,
	)
)
...
type DiscordVoiceConnectionGateway struct {
	...
	stateToGuild map[domain.PlayerStateID]snowflake.ID
	guildToState map[snowflake.ID]domain.PlayerStateID
...
func NewDiscordVoiceConnectionGateway(
	...
	return &DiscordVoiceConnectionGateway{
		session:      session,
		botID:        botID,
		stateToGuild: make(map[domain.PlayerStateID]snowflake.ID),
		guildToState: make(map[snowflake.ID]domain.PlayerStateID),
...
func (c *DiscordVoiceConnectionGateway) FindPlayerStateID(
	...
	guildID, err := snowflake.Parse(connectionInfo.GuildID)
	if err != nil {
		return nil
	}
...
func (c *DiscordVoiceConnectionGateway) Join(
	...
	connectionInfo discord.VoiceConnectionInfo,
) error {
	guildID, err := snowflake.Parse(connectionInfo.GuildID)
	if err != nil {
		return fmt.Errorf("invalid guild ID %q: %w", connectionInfo.GuildID, err)
	}
	channelID, err := snowflake.Parse(connectionInfo.ChannelID)
	if err != nil {
		return fmt.Errorf("invalid channel ID %q: %w", connectionInfo.ChannelID, err)
	}

	if voiceState, err := c.session.State.VoiceState(
		guildID.String(),
		c.botID.String(),
	); err == nil &&
		voiceState.ChannelID == channelID.String() {
