...
type LavalinkAudioGateway struct {
	...
	mu             sync.RWMutex
	currentEntries map[core.PlayerStateID]core.QueueEntry
}
...
func NewLavalinkAudioGateway(
	...
	gw := &LavalinkAudioGateway{
		...
		events:          events,
		currentEntries:  make(map[core.PlayerStateID]core.QueueEntry),
	}
	return gw
}
...
func (g *LavalinkAudioGateway) Stop(ctx context.Context, playerStateID core.PlayerStateID) error {
	guildID, err := g.voiceConnection.ResolveGuildID(playerStateID)
	if err != nil {
		return err
	}
...
func (g *LavalinkAudioGateway) Pause(ctx context.Context, playerStateID core.PlayerStateID) error {
	guildID, err := g.voiceConnection.ResolveGuildID(playerStateID)
	if err != nil {
		return err
	}
...
func (g *LavalinkAudioGateway) Resume(ctx context.Context, playerStateID core.PlayerStateID) error {
	guildID, err := g.voiceConnection.ResolveGuildID(playerStateID)
	if err != nil {
		return err
	}
...
func (g *LavalinkAudioGateway) onLavalinkTrackEnd(
	...
	g.events.Publish(context.Background(), core.TrackEndedEvent{
