...
func (r *LavalinkTrackResolver) ResolveQuery(
	...
	if node == nil {
		return core.TrackList{}, fmt.Errorf("no available Lavalink node")
	}

	result, err := node.LoadTracks(ctx, query)
	if err != nil {
		return core.TrackList{}, fmt.Errorf("failed to load tracks: %w", err)
	}

	switch data := result.Data.(type) {
	case lavalink.Track:
		return core.NewTrackList(
			core.TrackListTypeTrack,
			[]core.Track{r.trackCache.ConvertAndCache(data)},
		), nil

	case lavalink.Playlist:
		tracks := make([]core.Track, len(data.Tracks))
		...
		return core.NewTrackList(
			core.TrackListTypePlaylist,
			tracks,
			core.WithPlaylistInfo(identifier, data.Info.Name, cleanURL),
		), nil

	case lavalink.Search:
		tracks := make([]core.Track, len(data))
		for i, track := range data {
			tracks[i] = r.trackCache.ConvertAndCache(track)
		}
		return core.NewTrackList(
			core.TrackListTypeSearch,
			tracks,
		), nil

	case lavalink.Empty:
		return core.TrackList{}, fmt.Errorf("no results found")

	case lavalink.Exception:
		return core.TrackList{}, fmt.Errorf("lavalink load error: %w", data)
