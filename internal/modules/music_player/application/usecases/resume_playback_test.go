...
func TestResumePlaybackUsecase_Execute(t *testing.T) {
	type deps struct {
	...
	tests := []struct {
		...
		{
			name: "resume paused playback succeeds",
			deps: deps{
		...
		{
			name: "resume non-paused returns ErrNotPaused",
			deps: deps{
		...
		{
			name: "resume idle returns ErrNotPlaying",
			deps: deps{
				state: func(*testing.T) domain.PlayerState { return newIdleState() },
				locator: func(id domain.PlayerStateID) *stubPlayerStateLocator {
					return newStubLocator(map[string]domain.PlayerStateID{"guild1": id})
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			uc := NewResumePlaybackUsecase[string](
				newPlayerService(),
			...
			_, err := uc.Execute(context.Background(), ResumePlaybackInput[string]{
			...
			if len(audio.resumed) != tt.want.resumed {
				t.Errorf("Resume calls: got %d, want %d", len(audio.resumed), tt.want.resumed)
			}
